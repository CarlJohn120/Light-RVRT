...
try:
    ...
    DEFORM_EXT_AVAILABLE = False
    deform_attn_ext = None
    print('Warning: deform_attn extension not available, using Python fallback (slower and approximate).')
...
class DeformAttnFunction(Function):
    ...
    def forward(ctx,
        ...
        if DEFORM_EXT_AVAILABLE and deform_attn_ext is not None:
            ...
            # Fallback: return q (identity) so network can run without CUDA extension.
            return q
    ...
    def backward(ctx, grad_output):
        if DEFORM_EXT_AVAILABLE and deform_attn_ext is not None:
            ...
            # Fallback: approximate gradients as zeros (not used for inference)
            q, kv, offset = ctx.saved_tensors
            grad_q = torch.zeros_like(q)
            grad_kv = torch.zeros_like(kv)
            grad_offset = torch.zeros_like(offset)
